...
var imagesRouteOptions = {
  template: 'images',
  data: function () {
    var filter = imageFilters(Session.get('hostFilter'),Session.get('imageFilter'));
    return Images.find(filter, {sort: {Created: -1}, limit:Session.get('imagesLimit')});
  },
  onRun: function() {
  ...
  subscriptions: function() {
    return subs.subscribe('images', Session.get('imagesLimit'),Session.get('hostFilter'),Session.get('imageFilter'));
  }};
...
Router.route('/', {
  ...
  template: 'home',
  data: function () {
    var filter = containerFilters(Session.get('hostFilter'), Session.get('containerFilter'), Session.get('containerImgFilter'));
    return Containers.find(filter, {sort: {Created: -1}, limit:Session.get('containersLimit')});
  },
  onRun: function() {
  ...
  subscriptions: function() {
    var self = this;
    return [ subs.subscribe('containers', Session.get('containersLimit'), Session.get('hostFilter'), Session.get('containerFilter'), Session.get('containerImgFilter')),
...
Router.route('/images/:host/:id/tag', {
  ...
  data: function () {
    return ImagesInspect.findOne({_host:this.params.host, Id:queryImageId(this.params.id)});
  },
  onRun: function() {
    Meteor.call('image.details',this.params.host, this.params.id ,function(){});
    this.next()
...
Router.route('/containers/:host/:id/update', {
  name:'containerUpdate',
  template: 'containerUpdate',
  data: function () {
    return ContainersInspect.findOne({_host:this.params.host,Id:this.params.id});
  },
  onRun: function() {
  ...
  subscriptions: function() {
    var self = this;
    return [ Meteor.subscribe('containerInspect', this.params.host, this.params.id),
      modules.call('router.subscribe.container.update',self, this.params.host, this.params.id)];
  }
});
...
Router.route('/containers/:host/:id/attach', {
  name:'containerAttach',
  template: 'containerAttach',
  data: function () {
    return ContainersInspect.findOne({_host:this.params.host, Id:this.params.id});
  },
  onRun: function(){
    Meteor.call('container.attach',this.params.host,this.params.id ,function(err,wsKey){
	if (err){
	  console.log("attach",err);
	  Session.set('attachKey',undefined);
	}
      else
	Session.set('attachKey',wsKey);
    });
    this.next();
  },
  subscriptions: function() {
    var self = this;
    return [ Meteor.subscribe('containerInspect', this.params.host, this.params.id),
      modules.call('router.subscribe.container.attach',self, this.params.host, this.params.id)];
  }
});
...
Router.route('/containers/:host/:id/exec/:eid/attach', {
  name:'execAttach',
  template: 'execAttach',
  data: function () {
    var container = ContainersInspect.findOne({_host:this.params.host, Id:this.params.id});
    if (container)
      container.exec = this.params.eid;
    return container;
  },
  subscriptions: function() {
    var self = this;
    return [ Meteor.subscribe('containerInspect', this.params.host, this.params.id),
      modules.call('router.subscribe.exec.attach',self, this.params.host, this.params.id)];
  }
});
